...
export async function createOrUpdateCharacterLink(discordUserId: string, actorUuid: string, actorName: string): Promise<CharacterLink> {
    ...
    const response = await fetchFn(`${relayBase}/api/discord/links`, {
        method: 'POST',
        headers: { 'x-api-key': config.foundryApiKey, 'Content-Type': 'application/json' },
        body: JSON.stringify({ discordUserId, actorUuid, actorName })
    });
    ...
    if (!result.success) throw new Error('Failed to create/update character link');
...
export async function getCharacterLink(discordUserId: string): Promise<CharacterLink | null> {
    ...
    const result = await response.json() as { success: boolean; data: CharacterLink };
    if (!result.success) throw new Error('Failed to get character link');
...
export async function getAllCharacterLinks(): Promise<CharacterLink[]> {
    ...
    const result = await response.json() as { success: boolean; data: CharacterLink[] };
    if (!result.success) throw new Error('Failed to get all character links');
